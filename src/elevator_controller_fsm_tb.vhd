...
begin
	...
	begin
		...
		w_up_down <= '0';  wait for k_clk_period;
            assert w_floor = x"3" report "bad up from floor4" severity failure;	
		-- go up the rest of the way
        w_up_down <= '1';  wait for k_clk_period;
            assert w_floor = x"4" report "bad up from floor4" severity failure;	
		-- stop at top
        wait for k_clk_period;
            assert w_floor = x"4" report "bad ceiling lim" severity failure;
        -- go all the way down DOWN (how many clock cycles should that take?)
        w_up_down <= '0';
            wait for k_clk_period *3;
            assert w_floor = x"1" report "bad down to floor1" severity failure;
